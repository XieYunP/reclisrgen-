...
class RuleParser:
    ...
    @staticmethod
    def generate_standard_entries(rules, syllables, coverage, settings):
        """ 
        使用贪心算法优化生成标准录音条目 - 优化版本
        """ 
        import time
        ...
        # 构建音素依赖关系图 - 使用优化版本
        print("构建音素依赖关系图...")
        coda_consonant_graph = RuleParser._build_coda_consonant_graph(rules) 
        
        # 优先覆盖关键音素组合
        print("覆盖关键音素组合...")
        RuleParser._cover_critical_combinations(standard_entries, rules, syllables, coverage) 
        
        # 使用贪心算法选择最优组合 - 使用优化版本
        print("获取未覆盖组合...")
        remaining_combos = RuleParser._get_remaining_combinations(coverage, coda_consonant_graph) 
        
        print(f"应用贪心算法选择最优组合，处理 {sum(len(consonants) for consonants in remaining_combos.values())} 个组合...")
        RuleParser._greedy_coverage(standard_entries, remaining_combos, rules) 
        
        # 补充单音节覆盖
        print("补充单音节覆盖...")
        RuleParser._add_missing_syllables(standard_entries, syllables, coverage, rules, settings) 
        
        end_time = time.time()
        print(f"标准生成算法完成，耗时: {end_time - start_time:.2f} 秒")
    ...
    def _add_missing_syllables(standard_entries, syllables, coverage, rules, settings):
        ...
        for syl, rule in rules.items():
            for idx, trans_group in enumerate(rule['transition']):
                if trans_group and trans_group[0] and idx not in coverage['transition'].get(syl, set()):
                    standard_entries.add(syl)
    ...
    def generate_recording_list(rules, settings):
        ...
        print("开始生成标准条目...") # 添加日志
        standard_entries = RuleParser.generate_standard_entries(rules, syllables, coverage, settings)
